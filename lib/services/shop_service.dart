// ─────────────────────────────────────────────────────────────────────────────
// shop_service.dart — Phase 3 (v2.7.0)
//
// All shop-related business logic. No UI. No state management.
//
// Responsibilities:
//   • Phone validation + OTP via Supabase Auth
...
enum OtpSendResult   { sent, invalidPhone, alreadyRegistered, networkError }
enum OtpVerifyResult { verified, invalid, expired, networkError }
...
class ShopService {
  ...
  Future<OtpSendResult> sendOtp(String phone) async {
    debugPrint('ShopService.sendOtp: ${normalisePhone(phone)}');
    try {
      await _client.auth.signInWithOtp(phone: normalisePhone(phone));
      debugPrint('ShopService.sendOtp: sent ✅');
      return OtpSendResult.sent;
    } on AuthException catch (e) {
      debugPrint('ShopService.sendOtp AuthException: ${e.message}');
      return OtpSendResult.networkError;
    } catch (e) {
      debugPrint('ShopService.sendOtp error: $e');
      return OtpSendResult.networkError;
    }
  }
  ...
  Future<OtpVerifyResult> verifyOtp({
    ...
    debugPrint('ShopService.verifyOtp: ${normalisePhone(phone)}');
    try {
      ...
      debugPrint('ShopService.verifyOtp: verified ✅');
      return OtpVerifyResult.verified;
    } on AuthException catch (e) {
      debugPrint('ShopService.verifyOtp AuthException: ${e.message}');
      final msg = e.message.toLowerCase();
      if (msg.contains('expired')) return OtpVerifyResult.expired;
      return OtpVerifyResult.invalid;
    } catch (e) {
      debugPrint('ShopService.verifyOtp error: $e');
  ...
  /// Returns shop_id on success. Throws [ShopAlreadyExistsException] on duplicate phone.
  Future<String> registerShop({
    ...
    debugPrint('ShopService.registerShop: $shopName / $city');
    ...
    try {
      await _client.from('shops').insert({
        ...
        'owner_phone':   normalisePhone(phone),
        'city':          city,
        'plan':          'trial',
